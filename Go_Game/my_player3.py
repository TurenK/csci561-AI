...
class MinMaxPlayer:
                ...
                if go.board[x][y] == piece_type:
                    ...
                    if len(liberty) <= 2:
                        for empty in liberty.keys():
                            if go.valid_place_check(empty[0], empty[1], piece_type) and go.valid_place_check(empty[0], empty[1], 3 - piece_type):
                                next_go = go.copy_board()
    ...
    def filter_corner_surrounding(self, go, possible_places, piece_type):
        # filter corner
        if (0, 0) in possible_places:
            possible_places.remove((0, 0))
        if (0, 4) in possible_places:
            possible_places.remove((0, 4))
        if (4, 0) in possible_places:
            possible_places.remove((4, 0))
        if (4, 4) in possible_places:
            possible_places.remove((4, 4))

        temp = deepcopy(possible_places)

        # filter danger surrounding
        for r in possible_places:
    ...
    def find_max_place(self, go, piece_type, possible_place, a, b, deep, real_piece_type):
        if go.game_end() or len(possible_place) == 0 or deep == 0:
